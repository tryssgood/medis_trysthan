    ...
    if (isset($_POST['action'])) {
        switch ($_POST['action']) {
            case 'add':
                ...
                $stmt = $conn->prepare("INSERT INTO tindakan (nm_tindakan, biaya, keterangan) VALUES (?, ?, ?)");
            ...
            case 'edit':
                ...
                $stmt = $conn->prepare("UPDATE tindakan SET nm_tindakan=?, biaya=?, keterangan=? WHERE id_tindakan=?");
...
if (!empty($search)) {
    $search_condition = "WHERE nm_tindakan LIKE '%$search%' OR keterangan LIKE '%$search%'";
}
            ...
            fetch(`get_treatment.php?id=${id}`)
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        document.getElementById('edit_keterangan').value = treatment.keterangan;
                        openModal('editModal');
                    }
                })
                .catch(error => console.error('Error:', error));
